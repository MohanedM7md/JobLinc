...
function ManageFollowers() {
  const { company, loading, fetchCompanyFollowers } = useCompanyStore();
    ...
    if (companyId) {
      (async () => {
        try {
          ...
          console.error(
            "Error occurred while fetching company followers:",
            err,
          );
        }
      })();
    ...
    <div className="flex flex-col items-center w-full h-dvh p-4 md:p-6 lg:p-8 xl:p-10">
      ...
      <div className="w-full max-w-7xl mx-auto shadow-sm rounded-xl p-4 md:p-6 lg:p-8 bg-white">
        <h3 className="text-xl md:text-2xl font-semibold text-gray-700 mb-4 md:mb-6 pb-2 border-b border-gray-200">
          <span className="text-crimsonRed font-semibold">{company?.name}</span>{" "}
          Community
        </h3>
