...
function SignInInformation() {
    ...
    return (
        <form onSubmit={isValidSubmit} className="flex flex-col w-80 items-start gap-3">
            ...
            <div className="relative w-full">
                <div className="relative mb-13 w-full">
                    <input value={passText} type={isHidden ? "password" : "text"} name="password" onChange={handleChange} onFocus={handleFocus} onBlur={handleFocusOut} id="password" placeholder=" " className="peer z-1 border w-full h-11 rounded-sm absolute pl-2 pr-12 hover:cursor-text focus:outline-black transition-all" required />
                    <label htmlFor="password" className={`absolute left-2 text-mutedSilver transition-all px-1 ${(!isEmpty.password || isFocusedPass) ? "top-[0px] text-[10px]" : "top-2 text-base"}`}>Password</label>
                    <button onClick={handleClick} className="z-2 absolute top-3 right-1 rounded-2xl text-[12px] border-0 px-1.5 text-warmBlack hover:cursor-pointer">{isHidden ? "Show" : "Hide"}</button>
                </div>
                {showErrorPassInvalid && <p className="text-red-800 text-[12px]">The password you provided must have at least 6 characters.</p>}

            </div>

            <div className="text-center">
                <Link to="/Signin/ForgotPassword" className="text-warmBlack px-2 font-semibold hover:underline hover:rounded-3xl">Forgot password?</Link>
                <Outlet />
            </div>
            <div className="flex items-center px-2">
                ...
                <div className="flex items-center">
                    <label htmlFor="default-checkbox" className="text-[14px] text-warmBlack">Keep me logged in</label>
                </div>
            </div>
