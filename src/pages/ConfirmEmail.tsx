...
import { confirmEmail, sendConfirmationEmail } from "../store/userSlice";
...
function ConfirmEmail()
{
    ...
    async function handleSubmit(otp: string)
    {
        ...
        setClearOTP(false);
        const response = await dispatch(confirmEmail({ email:  email, token: resendToken || token, otp: otp }));
    ...
    return (
        <div className="flex w-full h-screen items-center justify-center">
            {redirecting ? <p>Redirecting you back to main page...</p> : 
            <div className="flex flex-col p-4 bg-lightGray rounded-lg shadow-md">
                <h1 className="text-2xl font-bold">Confirm Email</h1>
                <p className="text-[16px]">Please enter the OTP sent to your email.</p>
                <OTPInput clear={clearOTP} onComplete={handleSubmit} />
            </div>}
            

            {isModalOpen && <Modal isOpen={isModalOpen} onClose={() => { setIsModalOpen(false); setClearOTP(true); }}>
                                <h1 className="text-2xl font-bold">Wrong OTP entered</h1>
                                <p onClick={handleResend} className="text-[16px] hover:underline hover:cursor-pointer">Resend confirmation email</p>
                            </Modal>}
        </div>
    );
