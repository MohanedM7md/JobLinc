  ...
  async function handleOTPCompletion(otp: string) {
    ...
    if (confirmOTP.fulfilled.match(resultAction)) {
      ...
      navigate("/reset-password", {
        state: {
          email: emailText,
          resetToken: resultAction.payload.resetToken,
        },
      });
    } else {
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-charcoalWhite m-0">
      ...
      <form onSubmit={isValidSubmit}>
        <div className="flex flex-col w-80 gap-4 bg-lightGray p-5 rounded-xl">
          ...
          <button
            ...
            id="back-btn"
            className="w-full bg-crimsonRed py-2 text-charcoalBlack font-semibold rounded-3xl mb-4 hover:bg-softRosewood hover:cursor-pointer"
          >
            Back
          </button>
        </div>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)}>
        <div className="flex flex-col gap-2">
          <h2 data-testid="success-message" className="font-bold text-[18px]">
          ...
          {showErrorOTP && (
            <p className="text-[15px] text-warmBlack">
              Wrong OTP entered. Kindly check your email again to verify OTP.
            </p>
          )}
          {showErrorOTP && (
            <p className="text-[15px] text-warmBlack hover:cursor-pointer hover:underline">
              Get a new OTP
            </p>
          )}
